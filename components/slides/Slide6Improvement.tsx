...
export default function Slide6Improvement() {
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-stretch gap-3 md:gap-2 max-w-6xl w-full px-2 md:px-0">
        {improvementPlanContent.phases.map((phase, index) => {
          ...
          return (
            <motion.div key={phase.term} variants={itemVariants} className="flex-1 flex flex-col md:flex-row items-stretch md:items-center">
              {/* Card */}
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 md:p-6 hover:border-teal-400/40 transition-all duration-300 flex-1 flex flex-col w-full">
                <div className={`inline-flex p-2 md:p-3 rounded-xl ${phase.color} mb-3 md:mb-4 flex-shrink-0`}>
              ...
              {!isLast && (
                <div className="hidden md:flex items-center justify-center flex-shrink-0 px-2 lg:px-3">
